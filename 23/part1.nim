...
for step in 0..9:
    ...
    if not nobodyAround(elf, elves):
      for j in 0..4:
        ...
        if not checkDirs.anyIt(addOffset(elf, it) in elves):
          ...
          let movers = proposedMoves.getOrDefault(dest)
          proposedMoves.del(dest)  # this seems to be necessary to avoid duplicate entries...?
          proposedMoves.add(dest, movers & @[elf])
          break
  ...
  for d,movers in proposedMoves:
    if movers.len == 1:
      let mover = movers[0]
      # echo fmt"Moving elf from {mover} to {d}"
      elves.excl(mover)
      elves.incl(d)
