...
proc without(x: int, keys: seq[int]): int =
  var y = x
  for k in keys:
    # echo fmt"removing {k} from {y:#b} (holds: {y.holds(k)})"
    y = y xor (1 shl k)
    # echo fmt"now: {y:#b} (holds: {y.holds(k)})"
  y
...
proc getNeighbours(v: Vertex): seq[Vertex] =
  ...
  if unturned.holds(myV) and unturned.holds(elephantV) and myV != elephantV:
...
func `<`(a, b: Vertex): bool =
  # a[2] >= b[2]
  let
    aPossible = 5*a[2] + 3*getApproximateOptimum(a[4], a[3])
    bPossible = 5*b[2] + 3*getApproximateOptimum(b[4], b[3])
  aPossible > bPossible
